...
def fogify_image(image, pollution_level=100, fog_image="none", fog_opacity=0.5):
    ...
    fogified_image = cv.convertScaleAbs(image,
                                        alpha=alpha,
                                        beta=beta)
    ...
    if fog_image != "none":
        ...
        fog = cv.resize(fog, (image.shape[1], image.shape[0]), interpolation=cv.INTER_NEAREST)
...
def add_frame_and_tab(image, pollution_level=100):
  ...
  if image_with_borders.width < image_with_borders.height:
    tab_coords = {
        "left" : image_with_borders.width*0.2 - (1 if int(image_with_borders.width*0.2) % 2 == 1 else 0),
        "right" : image_with_borders.width*0.8 + (1 if int(image_with_borders.width*0.8) % 2 == 1 else 0),
        "bottom" : image_with_borders.height,
        "top" : image_with_borders.height #this is to be edited in specific tabs
    }
  else:
    tab_coords = {
    "left" : image_with_borders.width*0.35 - (1 if int(image_with_borders.width*0.2) % 2 == 1 else 0),
    "right" : image_with_borders.width*0.65 + (1 if int(image_with_borders.width*0.8) % 2 == 1 else 0),
    "bottom" : image_with_borders.height,
    "top" : image_with_borders.height #this is to be edited in specific tabs
    }


  cropped_img = image_with_borders.crop((tab_coords["left"],
                                         tab_coords["top"] * height_factor,
                                         tab_coords["right"],
                                         tab_coords["bottom"]))

  # # the filter removes the alpha, you need to add it again by converting to RGBA
  blurred_img = cropped_img.filter(ImageFilter.GaussianBlur(blur_intensity)).convert("RGBA")
  ...
  image_with_borders.paste(blurred_img, 
...
def write_overlay_text(image, location, timestamp, pollution_level, typeface):
  ...
  aqi_text_font = ImageFont.truetype(typeface, find_font_size(aqi_text,
                                                              0.075, 
                                                              image.height, 
                                                              typeface))
  ...
  location_text_font = ImageFont.truetype(typeface, find_font_size(location_text, 
                                                                   0.035, 
                                                                   image.height, 
                                                                   typeface))
...
def process_image(image_path, location, pollution_level, original=False, debug=False):
    ...
    if original:
      ...
      fog_overlay_img = random.choice(os.listdir(os.path.join(os.getenv('FLASKR_ROOT'), "Fogs")))
      fog_opacity = 1 if ( log(1 / (pollution_level / 100) + 1) > 1 ) else log(1 / (pollution_level / 100) + 1)
